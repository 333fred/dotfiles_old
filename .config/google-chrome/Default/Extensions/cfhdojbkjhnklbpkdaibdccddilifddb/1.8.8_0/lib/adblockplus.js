...
require.scopes["whitelisting"] = (function()
{
  ...
  var verifyKey = function(key, signature, url, docDomain)
  {
    var uri = new URI(url);
    var params = [uri.path, uri.asciiHost + (uri.port != -1 ? ":" + uri.port : ""), window.navigator.userAgent];
    return verifySignature(key, signature, params.join("\x00"));
  };
  var recordKey = function(page, url, key)
  {
    var urlsWithKey = pagesWithKey.get(page);
    if (!urlsWithKey)
    {
  ...
  var processKey = exports.processKey = function(token, page, frame)
  {
    var url = stripFragmentFromURL(frame.url);
    var docDomain = extractHostFromURL((frame.parent || frame).url);
    if (token.indexOf("_") < 0)
    {
      return;
    }
    var _tempVar41 = token.split("_", 2);
    var key = _tempVar41[0];
    var signature = _tempVar41[1];
    key = key.replace(/=/g, "");
...
require.scopes["filterValidation"] = (function()
{
  ...
  var Filter = _tempVar42.Filter;
  var InvalidFilter = _tempVar42.InvalidFilter;
